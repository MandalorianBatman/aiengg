...
> A neural network trained to predict the next token of an input sequence. The output is appended to the input, and the next token is predicted again. This autoregressive loop produces the full response. Gaurav deliberately avoids the word "world" — the only objective is next-token prediction.
